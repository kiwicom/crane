...
@click.option('--project', '--env', envvar='RANCHER_PROJECT_ID', required=True)
@click.option('--stack', envvar='RANCHER_STACK_NAME', required=False)
@click.option('--service', envvar='RANCHER_SERVICE_ID', required=True, multiple=True)
...
def main(rancher_url, access, secret, env, stack, service, new_image, batch_size, batch_interval, start_first, sidekick,
         sleep_after_upgrade, no_finish_upgrade):

    for single_service in service:
        log = glog.bind(project=env, service=single_service, new_image=new_image, sidekick=sidekick)

        stack, single_service = ids.names_to_ids(rancher_url, env, stack, single_service, access, secret)

        launch_config, secondary_launch_config = utils.get_launch_configs(
            rancher_url, env, single_service, sidekick, new_image, access, secret
        )
        ...
        utils.upgrade(
            rancher_url, env, single_service,
            upgrade_request, sleep_after_upgrade, no_finish_upgrade, log, access, secret
        )
