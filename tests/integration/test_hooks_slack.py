...
def mock_slack_api(requests_mock):
    ...
    requests_mock.get(
        ...
        json={"channels": [{"name": "general", "id": "123"}, {"name": "team", "id": "42"}]},
    )
...
def test_get_existing_messages(monkeypatch, mocker, repo, slack_response, result):
    ...
    assert slack_hook.get_existing_messages() == [None, None]
    ...
    assert messages[0]["text"] == deployment_id
    assert messages[1]["attachments"][0][
        "fields"
    ] == uut.AttachmentFields([])
